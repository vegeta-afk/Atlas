...
const CancelList = () => {
  ...
  // Fetch cancelled admissions from backend
  const fetchCancelledAdmissions = async () => {
    try {
      setLoading(true);
      ...
      if (response.data.success) {
        const transformedAdmissions = response.data.data.map((admission) => ({
          ...
          admissionStatus: admission.status || "cancelled",
          email: admission.email,
          cancellationReason: admission.remarks || "No reason provided",
          cancelledAt: admission.updatedAt || admission.createdAt,
        }));
      ...
      console.error("Error fetching cancelled admissions:", err);
      setError(err.message || "Failed to load cancelled admissions");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReactivate = async () => {
    ...
    try {
      const response = await admissionAPI.reactivateAdmission(
        selectedStudent.id, 
        statusReason || "Reactivated from Cancel List"
      );
      
      if (response.data.success) {
        alert("Student reactivated successfully!");
        setShowStatusModal(false);
        fetchCancelledAdmissions(); // Refresh the list
      }
    } catch (error) {
  ...
  const getStatusBadge = (status) => {
    return (
      <span className="admission-status-badge bg-red-100 text-red-800">
        <XCircle size={12} />
        Cancelled
      </span>
    );
    ...
    <div className="admission-list-container">
      ...
      {loading && (
        <div className="loading-overlay">
          ...
          <p>Loading cancelled admissions...</p>
        </div>
      )}
      ...
      {error && !loading && (
        <div className="error-alert">
          <AlertCircle size={20} />
          <div>
            <strong>Error loading cancelled admissions:</strong>
            <p>{error}</p>
          </div>
            ...
            {showDateFilter && (
              ...
              >
                <div className="date-filter-header">
                  <h4>Filter by Cancelled Date</h4>
                ...
                <tr key={admission.id}>
                  ...
                  <td>
                    <div className="date-info">
                      <Calendar size={14} />
                      {formatDate(admission.cancelledAt)}
                    </div>
                  </td>
                  <td>
                    <div className="reason-cell" title={admission.cancellationReason}>
                      {admission.cancellationReason.length > 30
                        ? `${admission.cancellationReason.substring(0, 30)}...`
              ...
              <tr>
                <td colSpan="8" className="empty-row">
                  <div className="empty-state">
                    ...
                    <p>
                      {loading ? "Loading..." : "No students have been cancelled yet."}
                    </p>
                  </div>
                </td>
              </tr>
      ...
      {!loading && !error && pagination.totalPages > 1 && (
        <div className="pagination">
          <div className="pagination-info">
            ...
            {pagination.total} cancelled admissions
          </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <textarea
                ...
                placeholder="Why is this student being reactivated?"
                rows="3"
              />
            </div>
