...
import { examReportAPI } from "../services/api";

const UpcomingExamReport = () => {
  // State variables
  ...
  const [selectedExamNumber, setSelectedExamNumber] = useState("all");
  ...
  }, [debouncedSearch, selectedCourse, selectedFaculty, selectedBatch, selectedExamNumber, selectedCourseType, sortConfig.key, sortConfig.direction]);
  ...
  const fetchReportData = async () => {
    try {
      ...
      console.log("Fetching exam report with params:", params);

      const response = await examReportAPI.getUpcomingExamReport(params);
  ...
  const handleExport = async () => {
    try {
      const response = await examReportAPI.exportUpcomingExamReport();
      
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'upcoming-exams-report.csv');
  ...
  const getStatusBadge = (status, daysLeft) => {
    const statusMap = {
      "Critical": { color: "bg-red-100 text-red-800", icon: "⚠️" },
      "Very Soon": { color: "bg-orange-100 text-orange-800", icon: "🔥" },
      "Soon": { color: "bg-yellow-100 text-yellow-800", icon: "⏳" },
      "Approaching": { color: "bg-blue-100 text-blue-800", icon: "📅" },
      "Far": { color: "bg-green-100 text-green-800", icon: "✓" },
      "Due": { color: "bg-rose-100 text-rose-800", icon: "❗" },
      "Completed": { color: "bg-emerald-100 text-emerald-800", icon: "✅" },
    };

    const config = statusMap[status] || statusMap["Far"];
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Upcoming Exam Report</h1>
          <p className="text-gray-600">Track upcoming exams for all student courses</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-200 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Critical (≤15 days)</p>
            </div>
          </div>
          <div className="bg-white p-5 rounded-xl border border-gray-200 flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold text-gray-900">{stats.verySoon}</h3>
              <p className="text-sm text-gray-600">Very Soon (16-30 days)</p>
            </div>
          </div>
          <div className="bg-white p-5 rounded-xl border border-gray-200 flex items-center gap-4">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-gray-600">Generating exam report...</p>
        </div>
      )}
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-200 mb-6 flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-2 border border-gray-200 rounded-lg min-w-[150px]">
            ...
            >
              ...
              <option value="first">First Exam Only</option>
              <option value="second">Second Exam Only</option>
              <option value="third">Third Exam Only</option>
            </select>
          </div>
          ...
          <table className="w-full min-w-[1500px]">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th onClick={() => handleSort("dateOfExam")} className="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider cursor-pointer hover:bg-gray-100">
                  Exam Date {getSortIndicator("dateOfExam")}
                </th>
              ...
              {exams.length > 0 ? (
                exams.map((exam, index) => (
                  <tr key={exam.id || index} className={`hover:bg-gray-50 ${exam.courseType === 'additional' ? 'bg-purple-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-4 text-center text-sm font-medium text-blue-600">{exam.rollNo}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
                          {exam.studentName ? exam.studentName.charAt(0).toUpperCase() : "?"}
                        </div>
                        <span className="text-sm font-medium text-gray-900">{exam.studentName}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center justify-center gap-1.5 text-sm text-gray-600">
                        ...
                        {formatDate(exam.dateOfExam)}
                      </div>
                    </td>
                    <td className="px-4 py-4 text-center">
                      <span className={`font-bold ${
                        exam.daysLeft <= 15 ? "text-red-600" : 
                        exam.daysLeft <= 30 ? "text-orange-600" :
                        exam.daysLeft <= 60 ? "text-yellow-600" :
                        exam.daysLeft <= 90 ? "text-blue-600" : "text-green-600"
                      }`}>
                        {exam.daysLeft}
                      </span>
                    </td>
                ...
                <tr>
                  <td colSpan="11" className="py-16 text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-700 mb-1">No exam data found</h3>
      ...
      {!loading && !error && pagination.totalPages > 0 && (
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 p-4 bg-white rounded-xl border border-gray-200">
          <div className="text-sm text-gray-600">
            Showing {start} to {end} of {pagination.total} exams
          </div>
...
export default UpcomingExamReport;