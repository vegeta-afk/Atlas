...
const Login = () => {
  ...
  // shared: what happens after ANY successful login (password or Google)
  ...
  // Google sign-in: gets an access token from Google, backend verifies it
  // and checks whether the email is already linked to an account.
  const handleGoogleSignIn = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (err.response) {
          // this is where "email not registered" (404) lands
          setError(err.response.data?.message || `Server error: ${err.response.status}`);
        } else if (err.request) {
          setError("No response from server. Check if backend is running.");
        } else {
      ...
      <div className="flex items-center justify-center px-6 py-10 md:px-14 bg-white">
        <div className="w-full max-w-sm" style={{ animation: "riseIn 0.5s ease" }}>
          <div className="md:hidden mb-8 text-center">
            <img src={iitImage} alt="IIT Computer Institute" className="h-12 w-auto object-contain mx-auto mb-2" />
